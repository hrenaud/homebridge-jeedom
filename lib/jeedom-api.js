	...
	for (var i in cmds) {
		if (isset(cmds[i].generic_type)) {
			if (cmds[i].generic_type !== null) {
				...
				/*************** HEATING ***********************/
				case 'HEATING_STATE' :
					if (result_cmd.heating == undefined) {
						result_cmd.heating = {};
					}
					result_cmd.heating.state = cmds[i];
					break;
				case 'HEATING_TEMPERATURE' :
					if (result_cmd.heating == undefined) {
						result_cmd.heating = {};
					}
					result_cmd.heating.temperature = cmds[i];
					break;
				case 'HEATING_ON' :
					if (result_cmd.heating == undefined) {
						result_cmd.heating = {};
					}
					result_cmd.heating.heating_on = cmds[i];
					break;
				case 'HEATING_OFF' :
					if (result_cmd.heating == undefined) {
						result_cmd.heating = {};
					}
					result_cmd.heating.heating_off = cmds[i];
					break;
				case 'HEATING_OTHER' :
					if (result_cmd.heating == undefined) {
						result_cmd.heating = {};
					}
					result_cmd.heating.heating_other = cmds[i];
					break;
				/*************** ALARME ***********************/
				case 'ALARM_STATE' :
